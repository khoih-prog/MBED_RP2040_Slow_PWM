...
#ifndef PORTENTA_H7_SLOW_PWM_ISR_GENERIC_HPP
#define PORTENTA_H7_SLOW_PWM_ISR_GENERIC_HPP
...
int Portenta_H7_Slow_PWM_ISR::setupPWMChannel(uint32_t pin, uint32_t period, uint32_t dutycycle, void* cbStartFunc, void* cbStopFunc)
{
  int channelNum;
...
void Portenta_H7_Slow_PWM_ISR::restartChannel(unsigned channelNum) 
{
  if (channelNum >= MAX_NUMBER_CHANNELS) 
  {
    return;
...
bool Portenta_H7_Slow_PWM_ISR::isEnabled(unsigned channelNum) 
{
  if (channelNum >= MAX_NUMBER_CHANNELS) 
  {
    return false;
...
void Portenta_H7_Slow_PWM_ISR::enable(unsigned channelNum) 
{
  if (channelNum >= MAX_NUMBER_CHANNELS) 
  {
    return;
...
void Portenta_H7_Slow_PWM_ISR::disable(unsigned channelNum) 
{
  if (channelNum >= MAX_NUMBER_CHANNELS) 
  {
    return;
...
void Portenta_H7_Slow_PWM_ISR::toggle(unsigned channelNum) 
{
  if (channelNum >= MAX_NUMBER_CHANNELS) 
  {
    return;
...
unsigned Portenta_H7_Slow_PWM_ISR::getnumChannels() 
{
  return numChannels;
}

#endif    // PORTENTA_H7_SLOW_PWM_ISR_GENERIC_HPP
